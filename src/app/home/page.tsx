  ...
  return (
    ...
    >
      ...
      <div style={{ position: 'absolute', top: 10, right: 10, display: 'flex', gap: '1rem' }}>
        <button>Çıkış</button>
        <button>Para Yatır</button>
        <button>Para Çek</button>
      </div>

      {/* Üst sol profil */}
      <div style={{ position: 'absolute', top: 10, left: 10 }}>
        <button>👤 Profil (Mevcut Bakiye: 7600 TL)</button>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', gap: '2rem', marginTop: '3rem', flexWrap: 'wrap' }}>
        {[
          ...
          <div key={i} style={{ backgroundColor: 'rgba(255,255,255,0.1)', padding: '1rem', borderRadius: '12px', width: '250px', textAlign: 'center' }}>
            {neden}
          </div>
        ))}
      </div>
